    ...
    <div className="app">
      ...
      {showInstallBanner && installPrompt && (
        <div style={{ background: 'var(--bg2)', borderBottom: '0.5px solid var(--border)', padding: '10px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontSize: 13, color: 'var(--text)' }}>📱 Add SPY AI Trader to your home screen</div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={handleInstall} style={{ fontSize: 12, padding: '5px 12px', background: 'var(--green)', border: 'none', borderRadius: 'var(--radius)', color: '#0d1a0d', cursor: 'pointer', fontWeight: 600 }}>Install</button>
            <button onClick={() => setShowInstallBanner(false)} style={{ fontSize: 12, padding: '5px 10px', background: 'var(--bg3)', border: '0.5px solid var(--border2)', borderRadius: 'var(--radius)', color: 'var(--text2)', cursor: 'pointer' }}>✕</button>
          </div>
        </div>
      )}
